...
if (
	...
	throw new Error(`Packaging requires Node >=${expectedNode}; found ${process.version}`);
...
const npmCli = process.env.npm_execpath ?? join(dirname(executable), "node_modules", "npm", "bin", "npm-cli.js");
try {
	await cp(join(dirname(npmCli), ".."), join(output, "npm"), { recursive: true });
} catch {
	throw new Error(`Unable to bundle npm CLI from ${npmCli}`);
}
await writeFile(
